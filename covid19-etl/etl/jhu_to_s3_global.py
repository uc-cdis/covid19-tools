...
This ETL generates data for all the world countries, all the US states and all
the US counties, and uploads this data to S3. Deprecated in favor of the
"JHU_to_S3" ETL which only generates the Illinois data used by the frontend.
"""
...
"""
    ...
    Then, we use self.nested_dict to generate a JSON file with
    all the dates, sorted by county in IL only. (5)
    It's used to display the map data by date for IL.
    ...
    "C" for Confirmed and "D" for Deaths are used to reduce file size
    {
        # IL only
        "il_county_list": {
            <US county FIPS>: {
...
class JHU_TO_S3_GLOBAL(base.BaseETL):
            ...
            for province_data in country_data.get("provinces", {}).values():
                ...
                for county_data in province_data.get("counties", {}).values():
                    ...
                    for date, ts in county_data["time_series"].items():
                        ...
                        countyList[county_fips]["by_date"][date] = {
                            "C": replace_small_counts_simple(confirmed),
                            "D": replace_small_counts_simple(deaths),
                        }
