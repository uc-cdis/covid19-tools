...
class GOOGLE_MOBILITY(base.BaseETL):
    def __init__(self, base_url, access_token, s3_bucket):
        ...
        self.expected_headers = ['country_region_code','country_region','sub_region_1','sub_region_2','metro_area','iso_3166_2_code','census_fips_code','place_id','date','retail_and_recreation_percent_change_from_baseline','grocery_and_pharmacy_percent_change_from_baseline','parks_percent_change_from_baseline','transit_stations_percent_change_from_baseline','workplaces_percent_change_from_baseline','residential_percent_change_from_baseline']
        self.column_headers = {
            'census_fips_code': 6,
            'place_id': 7,
            'date': 8,
    ...
    def parse_row(self, row):
        ...
        self.nested_dict[self.column_headers['census_fips_code']].append({
            ...
            'daysElapsed': getDiffDaysSinceDataEpoch(datetime.strptime( row[self.column_headers['date']],'%Y-%m-%d')),
            'retail_and_recreation': row[self.column_headers['retail_and_recreation_percent_change_from_baseline']],
            'grocery_and_pharmacy': row[self.column_headers['grocery_and_pharmacy_percent_change_from_baseline']],
            'parks': row[self.column_headers['parks_percent_change_from_baseline']],
            'transit_stations': row[self.column_headers['transit_stations_percent_change_from_baseline']],
            'workplaces': row[self.column_headers['workplaces_percent_change_from_baseline']],
            'residential': row[self.column_headers['residential_percent_change_from_baseline']]
        })
    ...
    def publish_to_s3(self):
        ...
        self.s3_client.put_object(Body=str(json.dumps(self.nested_dict)), Bucket=self.s3_bucket, Key='google_mobility_data.json')
        print("Done!")
